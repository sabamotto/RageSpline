...
public class RageSplineEditor : Editor {
    ...
    public bool showMeshColliderWarning;
    public bool showLocalStyleOptions;
    ...
    public override void OnInspectorGUI() {
        ...
        if (rageSpline.DestroyFarseerNow) {
            foreach (Transform child in rageSpline.transform)
                if (child.gameObject.name.StartsWith("convexShape", System.StringComparison.Ordinal)) DestroyImmediate(child.gameObject);
            bool found = false;
            foreach (Transform child in rageSpline.transform)
                if (child.gameObject.name.StartsWith("convexShape", System.StringComparison.Ordinal)) found = true;
    ...
    public void OnSceneGUI() {
        ...
        if (rageSpline.GetComponent<Renderer>() != null)
            EditorUtility.SetSelectedRenderState(
                rageSpline.GetComponent<Renderer>(),
                RageSpline.showWireFrameInEditor ? EditorSelectedRenderState.Wireframe : EditorSelectedRenderState.Highlight
            );
